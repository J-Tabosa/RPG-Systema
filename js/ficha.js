...
async function carregarDB(){
  ...
  Object.keys(custom).forEach(cat => { if(DB[cat]) DB[cat] = DB[cat].concat(custom[cat]); });
...
function populatePresetSelect(selectedId) {
  ...
  const html = `<option value="default">Padrão (Todo Conteúdo)</option>` + 
    listaPresets.map(p => {
      const nomePreset = p.name || p.nome || 'Preset Sem Nome';
      return `<option value="${p.id}" ${selectedId === p.id ? 'selected' : ''}>${nomePreset}</option>`;
    }).join('');
...
function setAccentPreset(i){ upd(f=>{ if(!f.colors)f.colors={}; f.colors.accentIdx=i; delete f.colors.customAccent; }); applyFichaColors(); }
function setCustomAccent(val){ upd(f=>{ if(!f.colors)f.colors={}; f.colors.customAccent=val; delete f.colors.accentIdx; }); applyFichaColors(); }
function setBgPreset(i){ upd(f=>{ if(!f.colors)f.colors={}; f.colors.bgIdx=i; delete f.colors.customBg; }); applyFichaColors(); }
function setCustomBg(val){ upd(f=>{ if(!f.colors)f.colors={}; f.colors.customBg = val; delete f.colors.bgIdx;}); applyFichaColors(); }
function setCustomFieldFontColor(val){ upd(f=>{ if(!f.colors)f.colors={}; f.colors.customFieldFontColor = val; }); applyFichaColors(); }
function resetColors(){ upd(f=>{ f.colors={}; }); applyFichaColors(); toast('Cores restauradas ao padrão.'); }
...
function typeColor(t){ return '#5080b0'; }
...
function newFichaObj(name='Novo Personagem', presetId='default'){
  return {id:uid(),name,class:'',subclass:'',race:'',level:1,background:'',alignment:'',type:'player',
    presetId:presetId,
    profBonus:2,colors:{},
...
function confirmSkill(){
  ...
  if(!name)return;
  upd(f=>{f.skills.push({id:uid(),name,attr,prof:0});}); closeM('mSkill'); renderEditor(); toast('Habilidade adicionada!');
}
...
function confirmSpell(){
  ...
  const sp={name,level:parseInt(document.getElementById('spLevel').value)||1,
    ...
    range:document.getElementById('spRange').value.trim(),aoe:document.getElementById('spAoE').value.trim(),
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape')['mPreset','mAttr','mHabil','mSpell','mEquip','mSkill','mCombat'].forEach(closeM);
