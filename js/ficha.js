...
function loadCustom() {
  try {
    return (
      JSON.parse(localStorage.getItem(SK_CUSTOM)) || {
        racas: [],
        classes: [],
        profissoes: [],
        personalidades: [],
...
document.addEventListener("click", (e) => {
  if (
    themeMenuOpen &&
    !document.getElementById("themeMenuWrap").contains(e.target)
  ) {
    themeMenuOpen = false;
...
async function carregarDB() {
  try {
    ...
    console.warn("personagens.json não encontrado.", e);
  }
...
function getFilteredCat(cat, presetId) {
  ...
  let presets = [];
  try {
    presets = JSON.parse(localStorage.getItem(SK_PRESETS)) || [];
  } catch (e) {}

  const currentPreset = presets.find((p) => p.id === presetId);
  if (currentPreset && currentPreset.blocked && currentPreset.blocked[cat]) {
    return DB[cat].filter(
      (item) => !currentPreset.blocked[cat].includes(item.id)
    );
  }
  return DB[cat];
}
...
function openPresetModalForNew() {
  ...
  document.getElementById("mPresetTitle").textContent =
...
function populatePresetSelect(selectedId) {
  let listaPresets = [];
  try {
    listaPresets = JSON.parse(localStorage.getItem(SK_PRESETS)) || [];
  } catch (e) {}
  ...
  const html =
    ...
    listaPresets
      .map((p) => {
        const nomePreset = p.name || p.nome || "Preset Sem Nome";
...
function confirmPresetModal() {
  ...
  if (presetModalTargetFichaId === null) {
    ...
    upd((f) => {
      ...
      const validRacas = getFilteredCat("racas", chosenPresetId).map(
        (x) => x.nome
      );
      const validClasses = getFilteredCat("classes", chosenPresetId).map(
        (x) => x.nome
      );
      const validProfs = getFilteredCat("profissoes", chosenPresetId).map(
        (x) => x.nome
      );
...
function applyFichaColors() {
  ...
  if (document.getElementById("customBg")) {
    if (c.customBg)
      document.getElementById("customBg").value = c.customBg.slice(0, 7);
    else
  ...
  if (document.getElementById("customFieldFontColor")) {
    document.getElementById("customFieldFontColor").value =
      c.customFieldFontColor || (dark ? "#ffffff" : "#111111");
  }

  if (document.getElementById("fichaNameLabel"))
...
function setAccentPreset(i) {
  upd((f) => {
    if (!f.colors) f.colors = {};
    f.colors.accentIdx = i;
    delete f.colors.customAccent;
  });
  applyFichaColors();
}
function setCustomAccent(val) {
  upd((f) => {
    if (!f.colors) f.colors = {};
    f.colors.customAccent = val;
    delete f.colors.accentIdx;
  });
  applyFichaColors();
}
...
function setCustomBg(val) {
  upd((f) => {
    if (!f.colors) f.colors = {};
    f.colors.customBg = val;
    delete f.colors.bgIdx;
  });
  applyFichaColors();
}
function setCustomFieldFontColor(val) {
  upd((f) => {
    if (!f.colors) f.colors = {};
    f.colors.customFieldFontColor = val;
  });
  applyFichaColors();
}
...
function getAttrVal(f, id) {
  const a = f.attrs.find((x) => x.id === id);
  return a ? a.val : 10;
}
function skillBonus(f, s) {
  const base = mod(getAttrVal(f, s.attr)),
    pb = f.profBonus || 2;
...
function getFicha() {
  return fichas.find((f) => f.id === activeId);
}
function upd(fn) {
  const f = getFicha();
  if (!f) return;
...
const DEF_SLOTS = Array.from({ length: 9 }, (_, i) => ({
  level: i + 1,
  total: 0,
  used: 0,
}));
...
function folderFromType(t) {
  return t === "player" ? "player" : "npc";
}
function countFolder(fid) {
  if (fid === "all") return fichas.length;
  return fichas.filter((f) => (f.folder || folderFromType(f.type)) === fid)
    .length;
}
...
async function renderSidebar() {
  ...
  const tabsHtml = `<div style="display:flex;flex-direction:column;gap:3px;margin-bottom:10px">
    ${FOLDERS.map(
      (fo) => `<div onclick="setFolder('${
        fo.id
      }')" style="display:flex;align-items:center;gap:7px;padding:6px 8px;border-radius:5px;border:1px solid ${
        activeFolder === fo.id ? "var(--gold)" : "transparent"
      };background:${
        activeFolder === fo.id ? "rgba(200,168,107,0.1)" : "transparent"
      };cursor:pointer;color:${
        activeFolder === fo.id ? "var(--gold)" : fo.color
      };transition:all 0.15s">
      <i class="ti ${fo.icon}" style="font-size:13px"></i>
      <span style="font-family:'Cinzel',serif;font-size:10px;letter-spacing:1px;text-transform:uppercase;flex:1">${
        fo.label
      }</span>
      <span style="font-family:'Cinzel',serif;font-size:10px;color:var(--border)">${countFolder(
        fo.id
      )}</span>
    </div>`
    ).join("")}
  ...
  const filtered =
    activeFolder === "all"
      ? fichas
      : fichas.filter(
          (f) => (f.folder || folderFromType(f.type)) === activeFolder
        );
  if (!fichas.length) {
    el.innerHTML =
      tabsHtml +
      '<div class="empty-state" style="padding:16px 0"><div class="big">📜</div>Nenhuma ficha.</div>';
    return;
  }
  if (!filtered.length) {
    el.innerHTML =
      tabsHtml +
      '<div style="color:var(--muted);font-size:13px;font-style:italic;text-align:center;padding:16px 0">Nenhuma ficha nesta pasta.</div>';
    return;
  }
  ...
  const itemsHtml = await Promise.all(
    filtered.map(async (f) => {
      ...
      <div class="fi-info">
        ...
        <div class="fi-sub">${[f.race, f.class, f.level ? "Nv " + f.level : ""]
          .filter(Boolean)
          .join(" · ")}</div>
      </div>
      <div class="fi-actions">
        <button class="btn xs text" title="Alterar Preset de Campanha" onclick="event.stopPropagation();openPresetModalForEdit('${
          f.id
        }')"><i class="ti ti-edit"></i></button>
        <button class="btn xs danger fi-del" onclick="event.stopPropagation();delFicha('${
          f.id
...
async function renderEditor() {
  ...
  const attrsHtml =
    f.attrs
      .map(
        (a) => `
    <div class="attr-card">
      <button class="rm-btn" onclick="removeAttr('${
        a.id
      }')"><i class="ti ti-x"></i></button>
      <div class="attr-abr" onclick="editAttr('${a.id}')">${a.abr}</div>
      <div><input class="attr-val-input" type="number" value="${
        a.val
      }" min="1" max="30"
        onchange="setAttr('${a.id}',this.value)"
        oninput="document.getElementById('amod_${
          a.id
        }').textContent=fmod(parseInt(this.value)||10)"></div>
  ...
  const skillsHtml = f.skills
    .filter((s) => !s.isSave)
    .map((s) => {
      const bv = skillBonus(f, s),
        at = f.attrs.find((x) => x.id === s.attr);
      ...
      <button style="background:none;border:none;color:var(--red2);cursor:pointer;font-size:11px;padding:0 2px;opacity:0" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0" onclick="removeSkill('${
        s.id
      }')"><i class="ti ti-x"></i></button>
    </div>`;
    })
  ...
  const slotsHtml = f.spellSlots
    .map(
      (sl, i) => `
    <div class="spell-level-row">
      <span class="slot-lbl">Círculo ${sl.level}</span>
      <input class="slot-num" type="number" value="${
        sl.total
      }" min="0" max="9" onchange="setSlotTotal(${i},this.value)">
      <div class="spell-dots">${Array.from(
        { length: sl.total },
        (_, k) =>
          `<div class="spell-dot ${
            k < sl.used ? "used" : "avail"
          }" onclick="toggleSlot(${i},${k})"></div>`
      ).join("")}</div>
    </div>`
    )
    .join("");
    ...
    .map((h, i) => {
      const dots =
        h.uses > 0
          ? Array.from(
              ...
              (_, k) =>
                `<div class="use-dot ${
                  k < (h.usesSpent || 0) ? "used" : ""
                }" onclick="toggleUse(${i},${k})"></div>`
            ).join("")
          : "";
      ...
      <div class="habil-head" onclick="document.getElementById('hitem_${i}').classList.toggle('expanded')">
        ...
        <div class="habil-badges">
          ${
            h.level > 1
              ? `<span class="habil-badge lvl">Nv ${h.level}</span>`
              : ""
          }
          <span class="habil-badge uses">${
            h.uses > 0
              ? `${h.uses - (h.usesSpent || 0)}/${h.uses} usos`
              : "Ilimitado"
          }</span>
        </div>
      ...
      ${
        h.uses > 0
          ? `<div class="uses-tracker">${dots}<button class="btn xs print-hidden" style="margin-left:6px" onclick="resetUses(${i})" title="Resetar">↺</button></div>`
          : ""
      }
    </div>`;
    })
    ...
    <div class="equip-item" id="eqitem_${i}">
      ...
      <div class="equip-main">
        <div class="equip-name-row">
          ...
          <span class="equip-tag ${e.cat || "item"}">${
        e.cat === "consumivel"
          ? "Consumível"
          : e.cat === "equipamento"
          ? "Equipamento"
          ...
          ${
            e.weight > 0
              ? `<span style="font-size:11px;color:var(--muted)">${e.weight}kg</span>`
              : ""
          }
          ${
            e.qty > 1
              ? `<span style="font-size:11px;color:var(--muted)">×${e.qty}</span>`
              : ""
          }
        </div>
        ${
          e.desc
            ? `<div class="equip-meta expand-toggle" onclick="document.getElementById('eqitem_${i}').classList.toggle('expanded')">▸ ${e.desc.slice(
                0,
                50
              )}${
                e.desc.length > 50 ? "..." : ""
              }</div><div class="equip-desc">${e.desc}</div>`
            : ""
        }
      </div>
  ...
  editor.innerHTML = `
        ...
        <div class="character-header-grid-compact">
          <div class="avatar-compact-area print-hidden">
            <div class="avatar-compact-preview" id="editorAvatarPreview" onclick="document.getElementById('avatarFileInput').click()" title="Alterar Ícone">
              ${
                characterImg
                  ? `<img src="${characterImg}" style="width:100%;height:100%;object-fit:cover;">`
                  : '<i class="ti ti-user" style="font-size:32px;color:var(--muted)"></i>'
              }
            </div>
            ${
              characterImg
                ? `<button class="btn text danger xs" onclick="removeAvatar()" style="padding:2px; font-size:10px;"><i class="ti ti-trash"></i></button>`
                : ""
            }
          ...
          <div class="avatar-compact-preview print-only-avatar-compact" style="display:none;">
            ${
              characterImg
                ? `<img src="${characterImg}" style="width:80px;height:80px;border-radius:50%;object-fit:cover;">`
                : ""
            }
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:12px; flex:1">
            <div style="display:grid;grid-template-columns: 1fr; gap:12px;" class="meta-grid">
              <div class="form-field" style="margin:0">
                ...
                <input type="text" class="char-name-header" value="${
                  f.name
                }" onchange="setField('name',this.value);renderSidebar()" style="font-family:'Cinzel',serif;font-size:18px; padding: 6px 10px;">
              </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;" class="meta-grid">
              ...
              <div class="form-field" style="margin:0">
                ...
                <input type="text" value="${
                  f.subclass || ""
                }" onchange="setField('subclass',this.value)" placeholder="Ex: Ladino..." style="padding: 6px 10px;">
              </div>
              <div class="form-field" style="margin:0">
                <label>Raça</label>
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px">
              <div class="form-field" style="margin:0"><label>Nível</label><input type="number" value="${
                f.level
              }" min="1" max="20" onchange="setField('level',parseInt(this.value));autoProf()" style="padding: 6px 10px;"></div>
              <div class="form-field" style="margin:0"><label>Bônus</label><input type="number" value="${
                f.profBonus
              }" min="2" max="6" onchange="setField('profBonus',parseInt(this.value));renderEditor()" style="padding: 6px 10px;"></div>
              <div class="form-field" style="margin:0"><label>Alinhamento</label><input type="text" value="${
                f.alignment || ""
              }" onchange="setField('alignment',this.value)" placeholder="Leal e Bom..." style="padding: 6px 10px;"></div>
              <div class="form-field" style="margin:0"><label>Tipo</label>
                <select onchange="setField('type',this.value);renderSidebar();renderEditor()" style="padding: 6px 10px;">
                  <option value="player" ${
                    f.type === "player" ? "selected" : ""
                  }>Jogador</option>
                  <option value="neutral" ${
                    f.type === "neutral" ? "selected" : ""
                  }>NPC</option>
                </select>
              </div>
            </div>
    ...
    <div class="pdf-page-block pdf-page-break">
      <div class="sec">
        ...
        ${
          habilHtml ||
          '<div style="color:var(--muted);font-size:13px;font-style:italic">Nenhum poder ainda.</div>'
        }
      </div>
      ...
      <div class="sec" style="margin:0">
        <div class="sec-head">
          ...
          <div style="display:flex;align-items:center;gap:8px">
            ${
              f.equipment.length
                ? `<span style="font-size:11px;color:var(--muted)">${f.equipment
                    .reduce((a, e) => a + (e.weight || 0) * (e.qty || 1), 0)
                    .toFixed(1)}kg</span>`
                : ""
            }
        ...
        ${
          equipHtml ||
          '<div style="color:var(--muted);font-size:13px;font-style:italic">Nenhum item ainda.</div>'
        }
      </div>
    </div>
          ...
          <div class="appearance-vertical-layout">
            ...
            <div class="full-body-wrapper">
              <div class="full-body-preview" id="editorFullBodyPreview">
                ${
                  fullBodyImg
                    ? `<img src="${fullBodyImg}" class="full-body-img-render">`
                    : '<div class="upload-placeholder"><i class="ti ti-photo"></i><div>Adicione a Arte de Corpo Inteiro</div></div>'
                }
              </div>
              <div class="full-body-actions print-hidden" style="display:flex; gap:6px; margin-top:8px; justify-content:center;">
                ...
                ${
                  fullBodyImg
                    ? `<button class="btn xs danger" onclick="removeFullBody()"><i class="ti ti-trash"></i></button>`
                    : ""
                }
              </div>
            ...
            <div class="form-field desc-visual-field">
              ...
              <textarea class="textarea-fixed" onchange="setField('appearanceDesc',this.value)">${
                f.appearanceDesc || ""
              }</textarea>
            </div>
          </div>
        ...
        <div class="text-sections-column">
          ...
          <div class="sec" style="margin:0">
            ...
            <div class="compact-fields-grid">
              <div class="form-field"><label class="field-label">Traços</label><textarea class="textarea-fixed compact" onchange="setField('traits',this.value)">${
                f.traits || ""
              }</textarea></div>
              <div class="form-field"><label class="field-label">Ideais</label><textarea class="textarea-fixed compact" onchange="setField('ideals',this.value)">${
                f.ideals || ""
              }</textarea></div>
              <div class="form-field"><label class="field-label">Vínculos</label><textarea class="textarea-fixed compact" onchange="setField('bonds',this.value)">${
                f.bonds || ""
              }</textarea></div>
              <div class="form-field"><label class="field-label">Defeitos</label><textarea class="textarea-fixed compact" onchange="setField('flaws',this.value)">${
                f.flaws || ""
              }</textarea></div>
            </div>
          </div>
          ...
          <div class="sec" style="margin:0; display:flex; flex-direction:column; gap:10px;">
            ...
            <div class="form-field" style="margin:0;"><label class="field-label">Notas Livres</label><textarea class="textarea-fixed history" onchange="setField('notes',this.value)">${
              f.notes || ""
            }</textarea></div>
          </div>
...
function removeAttr(id) {
  ...
  upd((f) => {
    f.attrs = f.attrs.filter((x) => x.id !== id);
  });
  renderEditor();
}
...
function confirmAttr() {
  ...
  const abr =
    document.getElementById("aAbr").value.trim().toUpperCase() ||
  ...
  if (editingAttr) {
    ...
    upd((f) => {
      f.attrs.push({ id: uid(), label: name, abr, val });
    });
  }
  closeM("mAttr");
  renderEditor();
  toast(editingAttr ? "Atualizado!" : "Adicionado!");
  editingAttr = null;
}
function toggleSave(attrId) {
  upd((f) => {
    const id = "sv_" + attrId,
      sv = f.skills.find((s) => s.id === id);
...
function removeSkill(id) {
  upd((f) => {
    f.skills = f.skills.filter((x) => x.id !== id);
  });
  renderEditor();
}
function openAddSkill() {
  const f = getFicha();
  if (!f) return;
  document.getElementById("skAttr").innerHTML = f.attrs
    .map((a) => `<option value="${a.id}">${a.abr} — ${a.label}</option>`)
    .join("");
...
function confirmSkill() {
  const name = document.getElementById("skName").value.trim(),
    attr = document.getElementById("skAttr").value;
  if (!name) return;
  upd((f) => {
    f.skills.push({ id: uid(), name, attr, prof: 0 });
  });
  closeM("mSkill");
  renderEditor();
...
function removeCombat(id) {
  ...
  upd((f) => {
    f.combat = f.combat.filter((x) => x.id !== id);
  });
  renderEditor();
}
function renameCombat(id) {
  ...
  if (n && n.trim()) {
    upd((f) => {
      const cs = f.combat.find((x) => x.id === id);
      if (cs) cs.label = n.trim();
    });
    renderEditor();
  }
}
...
function confirmCombat() {
  const label = document.getElementById("cbName").value.trim(),
    val = parseInt(document.getElementById("cbVal").value) || 0;
  if (!label) return;
  upd((f) => {
    f.combat.push({ id: uid(), label, val });
  });
  closeM("mCombat");
  renderEditor();
...
function confirmHabil() {
  ...
  closeM("mHabil");
  renderEditor();
  toast(editingHabil !== null ? "Atualizado!" : "Poder adicionado!");
  editingHabil = null;
}
function removeHabil(i) {
  upd((f) => {
    f.habilidades.splice(i, 1);
  });
  renderEditor();
}
...
function resetUses(i) {
  upd((f) => {
    if (f.habilidades[i]) f.habilidades[i].usesSpent = 0;
  });
  renderEditor();
}
...
function confirmEquip() {
  ...
  closeM("mEquip");
  renderEditor();
  toast(editingEquip !== null ? "Atualizado!" : "Item adicionado!");
  editingEquip = null;
}
...
function autoProf() {
  ...
  if (!f) return;
  const lv = f.level || 1,
    pb = lv <= 4 ? 2 : lv <= 8 ? 3 : lv <= 12 ? 4 : lv <= 16 ? 5 : 6;
  upd((f) => {
    f.profBonus = pb;
  });
  renderEditor();
...
async function exportarFichaAtiva() {
  ...
  const nomeArquivo =
    f.name.toLowerCase().replace(/[^a-z0-9]/gi, "_") + "_ficha.json";
